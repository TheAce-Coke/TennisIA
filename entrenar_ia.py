...
# --- 2. INGENIERÍA DE CARACTERÍSTICAS EXTRA ---
print("--- 2. Preparando cruce de datos (Oponente) ---")

# Preparamos la tabla de consulta del oponente
...
# Quitamos duplicados para evitar errores al cruzar
...
features_win = ['player_rank', 'opponent_rank', 'player_form', 'opponent_form', 'h2h_wins', 'h2h_total']
...
# Para juegos, usamos el promedio de juegos de ambos y el ranking (partidos desiguales = menos juegos)
features_games = ['player_rank', 'opponent_rank', 'player_avg_games', 'opponent_avg_games']
features_games.extend([c for c in df_final.columns if c.startswith('surface_')])

X_g = df_final[features_games]
y_g = df_final['match_games'] # Ahora predecimos el NÚMERO de juegos
