...
def calcular_total_juegos(score_str):
    """Convierte '6-4 7-6(5)' en el número 23"""
    if not isinstance(score_str, str) or 'RET' in score_str or 'W/O' in score_str:
        return np.nan
    
    total = 0
    # Buscamos patrones de sets tipo "6-4" o "7-6"
    # Ignoramos los números entre paréntesis de los tiebreaks
...
df_completo = df_completo.dropna(subset=['Player_1', 'Player_2', 'Winner'])

print("Calculando total de juegos por partido (esto puede tardar un poco)...")
# Convertimos la columna Score en un columna "Winner" (quien ganó)
# Pero necesitamos el TOTAL, que es independiente de quien gana.
# Usamos nuestra columna "Score" original (que a veces se llama 'score' o 'Score')
...
if col_score in df_completo.columns:
    ...
    print("⚠️ Advertencia: No encuentro la columna Score.")
    df_completo['total_games'] = np.nan

# Borramos partidos donde no pudimos calcular juegos (ej: retiros)
...
df_p1['match_games'] = df_completo['total_games'] # Nueva columna
...
df_p2['match_games'] = df_completo['total_games'] # Nueva columna
...
# B. Promedio de Juegos (¿Son sus partidos largos o cortos?)
...
df_modelo['player_avg_games'] = df_modelo.groupby('player_name')['match_games'].transform(
    lambda x: x.shift(1).rolling(window=10, min_periods=1).mean()
).fillna(22) # Si no sabemos, asumimos 22 juegos (estándar)

# Guardar
df_modelo.to_csv("atp_matches_procesados.csv", index=False)
print("\n¡Archivo 'atp_matches_procesados.csv' actualizado con JUEGOS!")